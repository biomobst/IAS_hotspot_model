...
for (dataset in c(datasets.mini)) {
  dataset_id <- dataset$dataset_id
  variables <- dataset$variables
  constraints <- dataset$constraints
  
  a <- download_layers(dataset_id, variables = variables, constraints = constraints, directory= dir)
  ...
  outdir <- paste(dir,"/datalayer.tiff/",sep="")
  writeRaster(b, glue("{outdir}{dataset_id}_{variables}.tif"), bylayer=TRUE)
}
