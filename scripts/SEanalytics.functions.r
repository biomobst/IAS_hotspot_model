...
read.and.extract <- function(data.table,species,stack, speciespath,stackpath,plotpath,outpath){
  ...
  #print(head(presence))
  absence <- data.table$absence.data[which(data.table$species == species)]
 # print(head(absence))
  ...
  #
  present.data <-read.csv2(paste(speciespath,presence,sep="/"), stringsAsFactors = F)
  names(present.data)[1] <-"ID"
  if(is.na(absence)){absence <- ""}
  if(!absence==""){
  absent.data <-read.csv2(paste(speciespath,absence,sep="/"), stringsAsFactors = F)
  ...
  for(i in pseudoabsence){
    temp <- read.csv2(paste(Speciespath,i,sep="/"),
                               sep=";", stringsAsFactors = F)
    names(temp) <- c("ID", "decimalLongitude","decimalLatitude")
...
run.random.forests <- function(species, selvar, indata.path,iterations.path){
 ...
 my.control[["train"]] <- all.occurance.iters[[rep]][[iter]]
...
#  RF.selected <- try(randomForest(.d[.train.index,.sel.var.index],.value[.train.index], 
  ...
  RF.selected <- try(randomForest(.d[.train.index,.sel.var.index],.value[.train.index],
                                  prox=TRUE,classwt = c("absent"=w.abs,"present"=w.pres)) )
...
#
 ...
 # }
  ...
  if(.store.model==FALSE){
    RF.selected <- NA}
  return(list("RF.selected" = RF.selected, 
              "predict.selected" = NA,
              "probs.sel" = probs.sel))
}
...
plot.maps <- function(species,indata.path,modelpath,plotpath, colors,brk){
 ...
 # linear Europe
  png(paste(plotpath,"/",species, ".lin.trainpoints.Eur.png",sep=""),  width = 180, height = 180, units = "mm", res=1200)
